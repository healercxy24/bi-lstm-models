...
# get training data
train_data, train_title, train_datanorm = trainset_process(train_path)
pca_data = pca(train_datanorm, M)
lower_input, lower_label = sliding_window_selection(pca_data, train_title, J, K)
lower_input = lower_input.permute(0, 2, 1).to(device)
lower_label = lower_label.permute(0, 2, 1).to(device)


# switch to numpy
lower_input = lower_input.cpu().numpy()
lower_label = lower_label.cpu().numpy()
# shuffle the data
state = np.random.get_state()
np.random.shuffle(lower_input)
np.random.set_state(state)
np.random.shuffle(lower_label)
# switch back to tensor
lower_input=torch.from_numpy(lower_input).cuda()
lower_label=torch.from_numpy(lower_label).cuda()


# split into train set and valid set
lower_seq_tensor =lower_input[0:16000, :]
lower_valid_seq_tensor = lower_input[16000:, :]
lower_label_tensor =lower_label[0:16000, :]
lower_valid_label_tensor = lower_label[16000:, :]


"""
pca_traindata : (16000, 4)
lower_seq_tensor : torch.Size([16000, 4, 68])
lower_label_tensor : torch.Size([16000, 4, 5])
...
upper_input, upper_label = sliding_window_upper_level(pca_data, train_title, J, L)
upper_input = upper_input.permute(0, 2, 1).to(device)


# switch to numpy
upper_input = upper_input.cpu().numpy()
upper_label = upper_label.cpu().numpy()
# shuffle the data
state = np.random.get_state()
np.random.shuffle(upper_input)
np.random.set_state(state)
np.random.shuffle(upper_label)
# switch back to tensor
upper_input=torch.from_numpy(upper_input).cuda()
upper_label=torch.from_numpy(upper_label).cuda()


# split into train set and valid set
upper_seq_tensor =upper_input[0:16000, :]
upper_valid_seq_tensor = upper_input[16000:, :]
upper_label_tensor =upper_label[0:16000]
upper_valid_label_tensor = upper_label[16000:]


"""
upper_seq_tensor : torch.Size([16000, 4, 36])
upper_label_tensor : torch.Size([16000, 1])
...
def fit(learning_rate, hidden_size, fc_layer_size, lstm_dropout):
    ...
    LowerModel = LowerLSTM(lower_input_size, lower_hidden_size, lstm_num_layers).to(device)
    UpperModel = UpperLSTM(upper_input_size, hidden_size, lstm_num_layers, fc_layer_size, lstm_dropout).to(device)
    ...
    return best_train_loss, best_model
...
study = optuna.create_study(study_name=('train_loss_optim'))  # Create a new study.
